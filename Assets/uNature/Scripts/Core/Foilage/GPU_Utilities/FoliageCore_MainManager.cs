...
namespace uNature.Core.FoliageClasses
{
    ...
    public sealed class FoliageCore_MainManager : FoliageMeshManager
    {
        ...
        protected override void Awake()
        {
            ...
            uNature.Core.Threading.UNThreadManager.InitializeIfNotAvailable();
                    ...
                    for (var z = 0; z < resolution; z++)
                    {
                        ...
                        for (var worldX = initWorldX; worldX < initWorldX + terrainAdjuster; worldX++)
                        {
                            for (var worldZ = initWorldZ; worldZ < initWorldZ + terrainAdjuster; worldZ++)
                            {
                                ...
                                {
                                    var prototype = (byte)prototypes[prototypeIndex].id;

                                    grassMap.SetDensity(mInstanceIndex, prototype, (byte)details[prototypeIndex][z, x]);
        ...
        public static void InitializeAndCreateIfNotFound()
        {
            ...
            GameObject instanceGO = new GameObject("Foliage Main Manager [DESTROY ONLY FROM FOLIAGE MANAGER WINDOW!!]");
